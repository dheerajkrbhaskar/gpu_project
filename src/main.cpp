...
namespace crack
{
    namespace
    {
        ...
        CpuBenchResult benchmarkCpu(const Image &input, bool useOpenMP, int repeat, unsigned char threshold)
        {
            ...
            output.summary.images = move(lastResult.images);
...
int main(int argc, char **argv)
{
    ...
    {
        cerr << "Missing required --input argument\n\n";
        printUsage(cerr, argv[0]);
        return 1;
    }
    ...
    try
    {
        ...
        const filesystem::path inputPath(config.inputPath);
        const Image inputImage = loadImageAsBgr(config.inputPath);
        const string timestamp = timestampString();
        ...
        printPerformanceSummary(array<ModeReportRow, 3>{
        ...
        const string outputLabel = modeLabel(config.mode);
